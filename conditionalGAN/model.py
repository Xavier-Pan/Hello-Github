...
class pix2pix(object):
    ...
    def train(self, args):
        ...
        for epoch in xrange(args.epoch):
            ...
            batch_idxs = min(len(data), args.train_size) // self.batch_size  #如果train_size比較小則用部分data train,否則以train資料夾中的data數目當作要train的總數

            for idx in xrange(0, batch_idxs):
                batch_files = data[idx*self.batch_size:(idx+1)*self.batch_size]
                
                #此處已假設data是兩張圖合併在一起.load_data()會將圖拆成左右兩半 
                #並合成一個256*256*channel*2的tensor "batch"(原本是256*512*channel)
    ...
    def test(self, args):
        ...
        for i, sample_image in enumerate(sample_images):
            ...
            samples = self.sess.run(
                self.fake_B_sample,#透過val資料夾中的data 直接以generator(sampler)產生影像
